    ...
    <motion.div
      ...
      className={`max-w-[85%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap break-words
        ${mine
          ...
          : 'self-start text-white/90 rounded-bl-md bg-white/[0.06] border border-white/10'}`}
...
export default function AdminAIAssistant({ products = [], orders = [], revenue = 0 }) {
        ...
        {open && (
          <motion.div
            ...
            className="fixed z-50 border-hairline rounded-[28px] overflow-hidden flex flex-col
                       ...
                       sm:w-[420px] max-h-[80vh] sm:max-h-[78vh] glass-dark"
            role="dialog"
            aria-modal="true"
            aria-label="مساعدة الإدارة"
          >
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-arabic text-base font-extrabold text-white leading-tight">
                  مساعدة الإدارة
                </div>
                <div className="text-[11px] text-white/55 truncate">
                  {products.length} منتج · {orders.length} طلب
                </div>
              </div>
              <button
                type="button"
            ...
            >
              ...
              {sending && (
                <motion.div
                  initial={{ opacity: 0, y: 6 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="self-start bg-white/[0.06] border border-white/10 rounded-2xl rounded-bl-md px-3 py-2"
                >
                  <TypingDots />
                </motion.div>
              )}
            ...
            <form
              ...
              className="flex items-center gap-2 p-3 border-t border-white/10 bg-white/[0.03]"
            >
              <input
                ref={inputRef}
                value={input}
                ...
                onKeyDown={onKeyDown}
                disabled={sending}
                placeholder="اكتبي سؤالك أو طلبك..."
                aria-label="اكتبي سؤالك"
                className="flex-1 bg-white/[0.06] border border-white/10 rounded-2xl px-4 py-2.5
